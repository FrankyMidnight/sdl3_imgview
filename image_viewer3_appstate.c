...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[])
{
    if(argc < 2)
    {
        SDL_Log("Usage : image_viewer3 DIRPATH");
        exit(EXIT_FAILURE);
    }
    puts(argv[0]);
    ///
    ...
    *appstate = state;
    ...
    state->dest_rect.h = 0.0f;
    state->dest_rect.w = 0.0f;
    state->dest_rect.x = 0.0f;
    state->dest_rect.y = 0.0f;
...
void load_dir(void *appstate)
{
    ...
    state->file_list = calloc(state->total_files, state->MAX_CHARS);
